...
if [[ -n "$DEVICE_ID" ]]; then
    echo -e "  ${CYAN}${BOLD}━━━ NEXT: Connect your vault (one-time setup) ━━━${NC}"
    echo ""
    echo -e "  Your Server Device ID:"
    echo -e "  ${YELLOW}${BOLD}${DEVICE_ID}${NC}"
    echo ""
    echo -e "  ${BOLD}Step 1:${NC} On your Mac/PC, open Obsidian"
    echo -e "         Settings → Life Companion → Vault Sync"
    echo -e "         Paste the Device ID above → Click ${BOLD}Connect${NC}"
    echo ""
    echo -e "  ${BOLD}Step 2:${NC} Open Syncthing on this server:"
    echo -e "         ${CYAN}http://$(hostname -I 2>/dev/null | awk '{print $1}' || echo 'server-ip'):8384${NC}"
    echo -e "         A popup will appear: \"New Device\" → Click ${BOLD}Add Device${NC}"
    echo ""
    echo -e "  ${BOLD}Step 3:${NC} In the same popup, under ${BOLD}Sharing${NC} tab,"
    echo -e "         check the box next to ${BOLD}Life Companion Vault${NC}"
    echo -e "         → Click ${BOLD}Save${NC}"
    echo ""
    echo -e "  ${BOLD}Step 4:${NC} Back in Obsidian, a popup may appear asking to"
    echo -e "         accept the folder share → Click ${BOLD}Accept${NC}"
    echo ""
    echo -e "  ${GREEN}After sync completes, restart the bot:${NC}"
    echo -e "  ${BOLD}$DC restart${NC}"
    echo ""
    echo -e "  ${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
else
    warn "Could not get Device ID."
    echo -e "  Find it at: http://localhost:8384 → Actions → Show ID"
    echo -e "  Then follow the sync steps in the README."
fi

echo ""
echo -e "  ${BOLD}Useful commands:${NC}"
echo -e "    Logs:       ${CYAN}$DC logs -f${NC}"
echo -e "    Health:     ${CYAN}curl localhost:3456/health${NC}"
echo -e "    Restart:    ${CYAN}$DC restart${NC}"
echo -e "    Syncthing:  ${CYAN}http://localhost:8384${NC}"
echo ""
